...
class Program
{
  ...
  {
    Parallel.For(0, 26, a => {
      byte[] password = new byte[5]; // Масив от байтове от 5 елемента 
      byte[] hash;                   // Масив от байтове
      ...
      var sha = System.Security.Cryptography.SHA256.Create(); // Обект sha 
        ...
        for (password[2] = 97; password[2] < 123; password[2]++) {
          for (password[3] = 97; password[3] < 123; password[3]++) {
            for (password[4] = 97; password[4] < 123; password[4]++) {
              ...
              hash = sha.ComputeHash(password); // Връща хеш на текущата парола, пример "bdasf"
